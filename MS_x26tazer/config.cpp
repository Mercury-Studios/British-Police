#include "cfgMagazines.hpp" // specific magazines for this taser
#include "cfgAmmo.hpp" // specific ammo for this taser
class CfgPatches
{
    class ms_x26 //class of our weapon. Below includes the classes we create so the game can include them.
 {
    ...
    requiredversion = 0.1;
    units[] = {};
    weapons[] = {"ms_tazer", "ms_tazer_yellow", "ms_tazer_green"};
...
www.benredmond.net
 ...
 - Silence here, don't have a clue where I found this config but thanks for the example :D
*/
...
class CfgWeapons//beginning of our cfgWeapons entry
{
    ...
    class ms_tazer: Pistol_Base_F //slaves off Arma's Pistol_Base_F class. The values defined below overwrite Pistol_Base_F, anything not defined is inherited. Consult the wiki/sample content for a full config with every possible value.
    {
        ...
        class Safe : UGL_F//A custom muzzle "Safe" which slaves off Arma's UGL_F muzzle
        {
            ...
            reloadTime = 2;//unnecessary copy-paste from above because we give the "safe" muzzle no possible magazines, but I included this anyway just incase.
            dispersion = 0.007000;
            optics = 1;
    ...
    class ms_tazer_yellow: Pistol_Base_F
    {
        scope = 2;
   ...
   class ms_tazer_green: Pistol_Base_F
    {
        scope = 2;
        ...
        class Library {
            libtextdesc = "A tazer employed by law enforcement around the world";
        };
        descriptionUse = "X26 Tazer";
