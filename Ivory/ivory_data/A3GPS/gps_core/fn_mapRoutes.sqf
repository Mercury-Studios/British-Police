...
gps_allCrossRoadsWithWeight = ["gps_allCrossRoadsWithWeight"] call ivory_gps_misc_fnc_hashTable_create;
gps_roadsWithConnected =  ["gps_roadsWithConnected"] call ivory_gps_misc_fnc_hashTable_create;
  ...
  if (count _connected > 1) then {
    {
      if(count (roadsConnectedTo _x) < 3) then {
        ...
        if([gps_roadsWithConnected,_rID] call ivory_gps_misc_fnc_hashTable_exists) then {
          ([gps_roadsWithConnected,_rID] call ivory_gps_misc_fnc_hashTable_find) pushBack _road;
        }else{
          [gps_roadsWithConnected,_rID,[_road]] call ivory_gps_misc_fnc_hashTable_set;
        };
      };
    }foreach ((_near - _connected) - [_road]);
  };
  
  _currentConnected = [gps_roadsWithConnected,str _road] call ivory_gps_misc_fnc_hashTable_find;
  if(isNil "_currentConnected") then {
      [gps_roadsWithConnected,str _road,_connected] call ivory_gps_misc_fnc_hashTable_set;
  }else{
...
{
  _connected = [gps_roadsWithConnected,str (_x select 0)] call ivory_gps_misc_fnc_hashTable_find;
