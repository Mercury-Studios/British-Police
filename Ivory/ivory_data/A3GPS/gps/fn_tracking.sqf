...
private _fn_findNextNode = {
	...
	_nearestRoadInFullPath = [getPosATL player,30,_fullPath] call ivory_gps_misc_fnc_nearestRoadInArray;
...
try {
	while {vehicle player distance _goal > 15} do { //this script thread will be destroyed when arrived 
		...
		private _metric = ["metric"] call ivory_gps_fncgetSetting;
		...
		if !(isNull _next_node) then {
			...
			if(_next_node_index_fullPath >= (count _fullPath - 2)) exitWith {
				[
					format [["STR_ROAD_ARRIVED_IN"] call ivory_gps_fnclocalize,[vehicle player distance _goal,0,_metric] call ivory_gps_misc_fnc_distanceStr],
					"A3\ui_f\data\Map\Markers\Military\flag_CA.paa",
					[vehicle player distance _goal,0] call ivory_gps_misc_fnc_distanceStr
				] call ivory_gps_menu_fnc_setGPSInfo;
			};
			...
			if (_diff > 0.3) then {
				[
						format[["STR_ROAD_TURN_RIGHT"] call ivory_gps_fnclocalize,[_distanceToNode,0,_metric] call ivory_gps_misc_fnc_distanceStr],
						gps_dir + "icons\direction_fork_right.paa",
						[vehicle player distance _goal,0,_metric] call ivory_gps_misc_fnc_distanceStr
				] call ivory_gps_menu_fnc_setGPSInfo;
			}else{
				if (_diff < -0.3) then {
					[
						format[["STR_ROAD_TURN_LEFT"] call ivory_gps_fnclocalize,[_distanceToNode,0,_metric] call ivory_gps_misc_fnc_distanceStr],
			...
			[
				["STR_GPS_LOST"] call ivory_gps_fnclocalize,
