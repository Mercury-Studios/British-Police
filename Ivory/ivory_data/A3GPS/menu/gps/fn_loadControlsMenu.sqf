...
_header ctrlSetText (["STR_MENU_CONTROLS_HEADER"] call ivory_gps_fnclocalize);
_del_btn ctrlSetText (["STR_MENU_CONTROLS_DEL_KEY_BTN"] call ivory_gps_fnclocalize);
_add_btn ctrlSetText (["STR_MENU_CONTROLS_ADD_KEY_BTN"] call ivory_gps_fnclocalize);

{
	_name = [getText (_x >> "name")] call ivory_gps_fnclocalize;
	_idx = _controls_list lbAdd _name;
	_controls_list lbSetData [_idx,configName _x];
}foreach ("true" configClasses (configFile >> "CfgGPS" >> "Keys"));

// seems weird but it's fine
_display setVariable ["refresh_key_list",{
	params ["_list","_class"];
	_allKeys = [_class,[]] call ivory_gps_fncgetSetting;
...
_add_btn ctrlAddEventHandler ["ButtonClick",{
	_this spawn { // need to spawn for the keyChoose dialog
		...
		_key = [_display] call ivory_gps_misc_fnc_keyChoose;
		if (_key == -1) exitWith {};
